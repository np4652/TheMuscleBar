...
namespace ApiRequestUtility
{
    public class AppWebRequest : IAppWebRequest
    {
        ...
        public async Task<string> CallUsingHttpWebRequest_GETAsync(string URL, int timoutinSecound, IDictionary<string, string> headers = null)
        {
            HttpWebRequest http = (HttpWebRequest)System.Net.WebRequest.Create(URL);

            http.Timeout = timoutinSecound ==0 ? 5 * 60 * 1000 : timoutinSecound*1000;
            string result = string.Empty;
            try
            {
                if (headers != null)
        ...
        public string CallUsingWebClient_GET(string URL, int timeout = 0)
        {
            ...
            try
            {
                ...
                result = appWebClient.DownloadString(URL);
            }
            catch (Exception ex)
            {
                throw;
        ...
        public async Task<string> CallUsingWebClient_GETAsync(string URL, int timeout = 0)
        {
            ...
            try
            {
                AppWebClient appWebClient = (timeout == 0) ? new AppWebClient() : new AppWebClient(timeout);
                appWebClient.Encoding = Encoding.UTF8;
                result = await appWebClient.DownloadStringTaskAsync(URL);
            }
            catch (Exception ex)
            {
                result = ex.Message;
